...
{
    public class Licence_Shop_NightShiftService : ILicence_Shop_NightShiftService
    {
        private readonly AppDbContext _context;
        private readonly IGenericRepository<Licence_Shop_NightShift_Approval> _iGR_Licence_Shop_NightShift_Approval;
        private readonly IApplicationManagementService<Licence_Shop_NightShift_Approval> _iApplicationMamnagementService;
        ...
        public Licence_Shop_NightShiftService(AppDbContext context,
            IGenericRepository<Licence_Shop_NightShift_Approval> iGR_Licence_Shop_NightShift_Approval,
            IApplicationManagementService<Licence_Shop_NightShift_Approval> IApplicationMamnagementService,
        ...
        //public async Task<GenericResponseTemplateModel<List<Licence_Shop_NightShift_ChecklistPoint>>> GetCheckListPoints(long id)
        //{
        //    GenericResponseTemplateModel<List<Licence_Shop_NightShift_ChecklistPoint>> genericServiceResultTemplate = new GenericResponseTemplateModel<List<Licence_Shop_NightShift_ChecklistPoint>>();
        //    try
        //    {
        //        genericServiceResultTemplate.HasError = false;
        //        genericServiceResultTemplate.ResponseDataModel = _context.Licence_Shop_NightShift_ChecklistPoints.Where(x=>x.IsEnabled==true).ToList();
        ...
        public async Task<GenericFormModel<Licence_Shop_NightShift_Approval>> GetGeneralDetail(Int64 id, Int64 projectSiteId)
        {
            GenericFormModel<Licence_Shop_NightShift_Approval> genericFormModel = new GenericFormModel<Licence_Shop_NightShift_Approval>();
            try
            {
                genericFormModel.FormModel = new Licence_Shop_NightShift_Approval();
                genericFormModel.FormModel.NightShift_ChecklistPoints = new List<Licence_Shop_NightShift_ChecklistPoint>();

                
                //genericServiceResultTemplate.ResponseDataModel = genericServiceResultTemplate.ResponseDataModel.Select(x => { x.IsSelected = false; return x; }).ToList();
                if (id != 0) //Existing Record
                {
                    var parentWithChildObject = await _iGR_Licence_Shop_NightShift_Approval
                       .GetAsync(x => x.AppRefId == id,  //Conditions         
                         null,                //Orders
                          //x => x.Licence_Shop_NightShift_ChecklistPoints,
                    ...
                    genericFormModel.FormModel.NightShift_ChecklistPoints = _context.Licence_Shop_NightShift_ChecklistPoints.Where(x => x.IsEnabled == true).ToList();
                ...
                genericFormModel.AppFormStepsList = await _iApplicationMamnagementService.GetAppFormStepperInfo(id, ApplicationTypeEnum.WOMEN_NIGHT_SHIFT_SHOP, (genericFormModel.FormModel != null ? genericFormModel.FormModel.Id : id), "GD");
            }
            catch (Exception ex)
            {
        ...
        public async Task<GenericServiceResultTemplate> AddUpdate_GeneralDetail(Licence_Shop_NightShift_Approval formModel, string id)
        {
            ...
            try
            {
                ...
                genericServiceResultTemplate.CustomeValidationResult = CustomeValidator<Licence_Shop_NightShift_Approval>.ValidateModel_AllProperties(formModel);
                if (genericServiceResultTemplate.CustomeValidationResult.IsValid)
                {
                    if (formModel.Id != 0) //Existing record
                    {
                        _iGR_Licence_Shop_NightShift_Approval.Update(formModel);
                        await _iGR_Licence_Shop_NightShift_Approval.SavechangeAsync();
                        ...
                        if (isStatusShared != null && !isStatusShared.HasError && !isStatusShared.ResponseDataModel)
                        {
                            ...
                            await _iThirdPartyInegrationsService.UpdateBusinessFirstRequestNativeAppIdByIPin(formModel.IPin, formModel.InvestPunjab_AppId, genericServiceResultTemplate.ApplicationInitiateResponse.AppId, ApplicationTypeEnum.WOMEN_NIGHT_SHIFT_SHOP, formModel.ApplicationPurposeType);

                            // Share status to Invest Punjab Portal
                            await _iThirdPartyInegrationsService.ShareStatusToBusinessFirst(genericServiceResultTemplate.ApplicationInitiateResponse.AppId, ApplicationTypeEnum.WOMEN_NIGHT_SHIFT_SHOP, AppActionTypeEnum.APP_SAVE_DRAFT);
                        }
                    ...
                    else //New record
                    {
                        ...
                        formModel = GenericModelOps<Licence_Shop_NightShift_Approval>.SetNullAllNevigationProperties(formModel);
                        genericServiceResultTemplate.ApplicationInitiateResponse = await _iApplicationMamnagementService.InitiateApplication(ApplicationTypeEnum.WOMEN_NIGHT_SHIFT_SHOP, formModel, formModel.ProjectSiteRefId, formModel.ApplicationPurposeType, id, formModel.IPin, formModel.InvestPunjab_AppId, false, 0, 0, null, null, formModel.ProjectSiteVersion);
        ...
        public async Task<GenericFormModel<Licence_Shop_NightShiftViewModel>> GetLicenceShopNightShiftDetail(long id)
        {
            GenericFormModel<Licence_Shop_NightShiftViewModel> genericFormModel = new GenericFormModel<Licence_Shop_NightShiftViewModel>();
            try
            {
                genericFormModel.ListTemplateLists = new List<ListTemplate>();
                genericFormModel.FormModel = new Licence_Shop_NightShiftViewModel();
                genericFormModel.FormModel.GeneralDetail = new Licence_Shop_NightShift_Approval();
                ...
                genericFormModel.FormModel.GeneralDetail.NightShift_ChecklistPoints = new List<Licence_Shop_NightShift_ChecklistPoint>();
                genericFormModel.FormModel.GeneralDetail.NightShift_ChecklistPoints = _context.Licence_Shop_NightShift_ChecklistPoints.Where(x => x.IsEnabled == true).ToList();
                ...
                genericFormModel.AppFormStepsList = new List<AppFormStepsInfo>();
                genericFormModel.AppFormStepsList = await _iApplicationMamnagementService.GetAppFormStepperInfo(genericFormModel.FormModel.GeneralDetail.AppRefId, ApplicationTypeEnum.WOMEN_NIGHT_SHIFT_SHOP, (genericFormModel.FormModel != null ? genericFormModel.FormModel.GeneralDetail.Id : id), "LOCK");
        ...
        public async Task<GenericServiceResultTemplate> LockApplication(Int64 appRefId, int AppActionType, string remarks)
        {
            ...
            try
            {
                ...
                if (appRefId != 0)
                {
                    ...
                    if (await _iApplicationMamnagementService.LockApplication(appRefId, AppActionType, remarks, ApplicationTypeEnum.WOMEN_NIGHT_SHIFT_SHOP) != "")
                    {
        ...
        public async Task<GenericResponseTemplateModel<List<GetlastClearanceViewModel>>> GetLastClearancesNTS(string licenceNo, Int64 projectSiteRefId, int projectSiteVersion)
        {
            ...
            try
            {
                ...
                var Clearance = await _iGeneric_SP_Repository.CallStoreProcedureReaderAsync<GetlastClearanceViewModel>("sp_GetLastCleranceNightShiftShop", storeProcedureParms);
